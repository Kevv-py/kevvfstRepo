...
export function CategoryChart({ expenses, currency, theme }: Omit<Props, 'month'>) {
  ...
  return (
    <div className="grid min-w-0 items-center gap-4 @md:grid-cols-[minmax(0,180px)_minmax(0,1fr)]">
      <div className="min-w-0">
      ...
      <ul className="grid min-w-0 gap-2">
        {data.map((entry) => (
          <li key={entry.category} className="flex min-w-0 items-center gap-2 text-sm">
            <span
              className="size-2.5 shrink-0 rounded-full"
              style={{ background: CATEGORY_COLORS[entry.category] ?? '#64748b' }}
            />
            <span className="truncate text-muted">{entry.category}</span>
            <span className="ml-auto shrink-0 font-medium tabular-nums">{formatMoney(entry.total, currency)}</span>
          </li>
        ))}
      </ul>
    </div>
